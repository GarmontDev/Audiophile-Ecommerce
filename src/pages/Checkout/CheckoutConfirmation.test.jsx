...
describe("Checkout Confirmation", async() => {
  ...
  const getCartTotal = vi.fn()
  getCartTotal.mockReturnValue(cartItems[0].price)
  
  render(
      <MemoryRouter initialEntries={[{ state: {cartItems} }]}>
          <CartContext.Provider value={{cartItems, getCartTotal}}>
  ...
  it("should display the name of the item in the cart", () => {
      expect(screen.getAllByText(cartItems[0].name)).toBeDefined()
  })

  it("should display the total price", () => {
      expect(screen.getAllByText(cartItems[0].price + " €")).toBeDefined()
  })
